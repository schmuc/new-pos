...
<?php
    ...
    if(isset($_POST['admin-myaccount-submit'])){
        ...
        $account = $_SESSION['account'];
        $admin->adminUpdateProfile($account['admin_id'], $profile, $firstname, $lastname, $username);
        $admin->adminUpdateSession($account['admin_id']);
        ...
        <div class="w-full flex justify-center">
            <form class="container border-show rounded-lg p-4 m-20 flex flex-col items-center w-2/5" method="post" enctype="multipart/form-data">
                <div class="border-show w-40 h-40 overflow-hidden">
                    <img id="admin-my-account-profile" src="../img/uploaded/<?php echo $account['image']?>" alt="no image" width="200">
                </div>   
                ...
                <div class="flex justify-between w-full mt-8">
                    ...
                    <input id="admin-ma-f" class="border-show rounded-lg p-2 w-1/2" type="text" value="<?php echo $account['firstname']?>" name="firstname">
                </div>
                <div class="flex justify-between w-full mt-8">
                    <label class="capitalize text-2xl" for="admin-ma-l">lastname:</label>
                    <input id="admin-ma-l" class="border-show rounded-lg p-2 w-1/2" type="text" value="<?php echo $account['lastname']?>" name="lastname">
                </div>
                <div class="flex justify-between w-full mt-8">
                    <label class="capitalize text-2xl" for="admin-ma-u">username:</label>
                    <input id="admin-ma-u" class="border-show rounded-lg p-2 w-1/2" type="text" value="<?php echo $account['username']?>" name="username">
                </div>
                <input class="button1 border-show w-28 rounded-lg py-1 mt-10" type="submit" value="Update" name="admin-myaccount-submit">
                <a class="mt-4 underline capitalize" href="admin-update-mypassword.php">change password</a>
            </form>
        </div>
    </section>
