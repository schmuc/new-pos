...
<?php
    ...
    $product = $admin->adminSpecificProduct($product_id)->fetch_assoc();
    ...
    if(isset($_POST['admin-product-submit'])){
        ...
        if($allIngredients == ''){
            ...
            $result = $admin->adminEditProduct($product_id , $image, $name, $category, $size, $allIngredients, $volume, $price, $cost, $stock);
                    
            move_uploaded_file($product_temp, "../img/uploaded/".$image);
    ...
    <section class="min-height-full relative">
        ...
        <div class="h-full flex flex-col items-center mt-20">  
            <form class=" w-4/5 flex flex-col px-4" method="post" enctype="multipart/form-data">
                <h1 class="uppercase font-black text-4xl">edit product</h1>
                <div class="mt-4 flex h-1/5">
                    <div>
                        <div class="grid place-items-center">
                            <img id="admin-add-product" class="border-show" accept="img/*" src="../img/uploaded/<?php echo $product['image'];?>" alt="no image" style="width: 300px; height: 200px;">
                        </div>
                    ...
                    <div class="w-full px-4 flex flex-col justify-center">
                        <div class="flex justify-between">
                            ...
                            <input class="border-show rounded-lg w-3/4 px-2" type="text" name="name" value="<?php echo $product['product'];?>">
                        </div>
                                ...
                                <select class="border-show  w-1/2 ml-4" name="category">
                                    <?php
                                        ...
                                        foreach($categories as $category){ 
                                            echo  $category['category'] == $product['category'] ? "<option selected value='$category[category]'>$category[category]</option>" : "<option value='$category[category]'>$category[category]</option>";
                                        }
                                    ?>
                                </select>
                            </div>
                                ...
                                <select class="border-show w-1/2 ml-4" name="size">
                                    <?php
                                        ...
                                        foreach($sizes as $size){
                                            echo  $size['size'] == $product['size'] ? "<option selected value='$size[size]'>$size[size]</option>" : "<option value='$size[size]'>$size[size]</option>";
                                        }
                                    ?>
                                </select>
                            </div>
                ...
                <div class="mt-8">
                    ...
                    <?php
                        ...
                        foreach($ingredients as $ingredient){
                            ...
                            echo str_contains($product['ingredient'], $ingredient['ingredient']) ? "<input class='mr-2' type='checkbox' checked value='$ingredient[ingredient]' name='$ingredient[ingredient]'>" : "<input class='mr-2' type='checkbox' value='$ingredient[ingredient]' name='$ingredient[ingredient]'>";                       
                ...
                <div class="mt-8 flex">
                    <div class="flex w-1/2">
                        ...
                        <input class="border-show rounded-lg w-2/6 px-2 ml-4" type="text" name="volume" value="<?php echo $product['volume'];?>">
                    </div>
                    <div class="flex w-1/2">
                        <label class="capitalize font-semibold text-4xl">price:</label>
                        <input class="border-show rounded-lg w-2/6 px-2 ml-4" type="text" name="price" value="<?php echo $product['price'];?>">
                    </div>
                </div>
                <div class="mt-8 flex">
                    <label class="capitalize font-semibold text-4xl">stock:</label>
                    <input class="border-show rounded-lg w-2/6 px-2 ml-4" type="number" name="stock" value="<?php echo $product['stock'];?>">
                </div>
                <div class="mt-8">
                    <div class=" text-end">
