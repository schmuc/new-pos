   ...
   include '../sql/queries.php';
    ...
    if(isset($_POST['admin-password-submit'])){
        ...
        if($newPassword == $retypeNewPassword){
            if($oldPassword == $adminSpecific['password']){
                $admin->adminUpdatePassword($adminID, $newPassword);
                echo "<script>alert('Password updated successfully'); window.location = 'superadmin-admin.php';</script>";
            }else{
        ...
        ?>
         <div class="w-full flex justify-center">
            <form class="container border-show rounded-lg p-4 m-20 flex flex-col items-center w-2/5" method="post">
                ...
                <input class="button1 border-show w-28 rounded-lg py-1 mt-10" type="submit" value="Update" name="admin-password-submit">
                <a class="mt-4 underline capitalize" href="superadmin-admin-edit.php?id=<?php echo $adminID?>">back</a>
            </form>
        </div>
    </section>
