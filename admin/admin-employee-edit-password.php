...
<?php
   include '../sql/queries.php';
    ...
    if(isset($_POST['admin-employee-password-submit'])){
        ...
        if($newPassword == $retypeNewPassword){
            if($oldPassword == $employee['password']){
                $staff->employeeUpdatePassword($employeeID, $newPassword);
                echo "<script>alert('Password updated successfully'); window.location = 'admin-employee.php';</script>";
            }else{
        ...
        ?>
         <div class="w-full flex justify-center">
            <form class="container border-show rounded-lg p-4 m-20 flex flex-col items-center w-2/5" method="post">
                ...
                <input class="button1 border-show w-28 rounded-lg py-1 mt-10" type="submit" value="Update" name="admin-employee-password-submit">
                <a class="mt-4 underline capitalize" href="admin-employee-edit.php?id=<?php echo $employeeID?>">back</a>
            </form>
        </div>
    </section>
